...
Resolves the vault from CLAUDE.local.md, extracts conversation context,
and spawns flush.py with the vault path so knowledge lands in the right place.

Uses offset tracking: each flush processes only NEW lines since the last flush,
so earlier turns are never lost even in long/restarted sessions.
"""
...
def extract_conversation_context(
    ...
    """Read JSONL transcript from offset and extract turns as markdown.

    Returns (context_text, turn_count, total_lines_read).
    """
    turns: list[str] = []
    line_number = 0
                    ...
                    if isinstance(block, dict):
                        ...
                        if btype == "text":
                            ...
                            if result_text.strip():
                                text_parts.append(
                                    f"[Result: {result_text[:300].strip()}]"
                                )
...
def main() -> None:
    ...
    logging.info(
        "SessionEnd fired: session=%s source=%s vault=%s",
        session_id,
        source,
        vault_path,
    )
    ...
    # Load offset for this session+vault combo
    state = load_state()
    ...
    try:
        context, turn_count, total_lines = extract_conversation_context(
            transcript_path, skip_lines
        )
    ...
    # Save offset BEFORE spawning flush (so next hook doesn't re-process)
    state[state_key] = {
    ...
    # Spawn flush.py with vault path as 3rd argument
